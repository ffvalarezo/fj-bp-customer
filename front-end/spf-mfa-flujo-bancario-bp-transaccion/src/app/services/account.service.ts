...
export class AccountService {
    ...
    getAccountsByAccountNumber(accountNumber: string): Observable<AccountRequest[]> {
        return this.http
            .get<AccountRequest[]>(`${this.baseUrl}/filter/movement/${accountNumber}`, {
                headers: this.getDefaultHeaders()
            })
            .pipe(
                catchError((error) => {
                    console.error('Error in getAccountsByAccountNumber:', error);
                    return this.handleApiError(error);
                })
            );
    }
