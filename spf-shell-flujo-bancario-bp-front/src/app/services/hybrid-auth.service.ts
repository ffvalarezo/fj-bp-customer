...
export class HybridAuthService {
  ...
  private isFullyAuthenticatedSubject = new BehaviorSubject<boolean>(this.hasValidBackendToken());
  public isFullyAuthenticated$ = this.isFullyAuthenticatedSubject.asObservable();

  constructor() {
    // Suscribirse a cambios de autenticación de Azure
    this.azureAuthService.getAccountState().subscribe((isAzureAuthenticated) => {
  ...
  isFullyAuthenticated(): boolean {
    return this.azureAuthService.isAuthenticated() && this.hasValidBackendToken();
  }
    ...
    this.azureAuthService.handleAuthRedirect().subscribe({
      next: (azureTokenResponse) => {
        if (azureTokenResponse) {
          const activeAccount = this.azureAuthService.msalService.instance.getActiveAccount();
  ...
  private exchangeAzureTokenForBackendJWT(request: BackendLoginRequest): void {
    this.http.post<BackendTokenResponse>(`${this.API_URL}/auth/azure-exchange`, request)
      .pipe(
        tap(response => {
          if (response.jwtToken) {
            ...
            if (response.permissions) {
              this.storageService.set(this.PERMISSIONS_KEY, JSON.stringify(response.permissions));
            }
