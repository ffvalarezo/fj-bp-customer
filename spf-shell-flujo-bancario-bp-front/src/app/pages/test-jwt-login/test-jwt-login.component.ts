...
export class TestJwtLoginComponent {
  ...
  onTestSubmit(): void {
    ...
    if (this.testLoginForm.valid) {
      ...
      console.log('Headers que se enviarán: x-guid, x-channel, x-medium, x-app, x-session');
  ...
  canSubmit(): boolean {
    ...
    const isValid = username && password && username.trim() !== '' && password.trim() !== '';
    console.log('Can submit:', isValid, 'Username:', username, 'Password:', password ? '***' : 'empty');
    return isValid;
  }
